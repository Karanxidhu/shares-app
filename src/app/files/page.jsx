...
function Files() {
  ...
  const fetchFiles = async () => {
    setLoading(true);
    const response = await fetch('https://sharesb.karanxd.space/api/message/files', {
      method: 'get',
      headers: {
  ...
  const submit = async (event) => {
    ...
    const result = await axios.post('https://sharesb.karanxd.space/api/message/upload', formData, { headers: { 'Content-Type': 'multipart/form-data' } })
    console.log(result)
    location.reload();
  }

  const handleDownload = async (key) => {
    const res = await fetch("https://sharesb.karanxd.space/api/message/download", {
      method: 'post',
      headers: {
        "token": localStorage.getItem('token'),
  ...
  const handleDelete = async (key) => {
    const response = await fetch('https://sharesb.karanxd.space/api/message/deletefile', {
        method: "post",
        headers: {
            "token": localStorage.getItem('token'),
            "Content-Type": "application/json"
